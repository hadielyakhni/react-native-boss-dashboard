  ...
  renderScreen() {
    if (!this.props.fetchingAccounts) {
      ...
      let pAccounts = matchedAccounts.filter(account => {
        const { status, amount1, amount2, amount3, amount4 } = account[1]
        if (status === 'ME')
          pTotal += (amount1 + amount2 + amount3 + amount4)
        return status === 'ME'
      })
      let nAccounts = matchedAccounts.filter(account => {
        const { status, amount1, amount2, amount3, amount4 } = account[1]
        if (status === "HIM")
          nTotal += (amount1 + amount2 + amount3 + amount4)
        return status === 'HIM'
      })
      return (
        <ScrollView>
