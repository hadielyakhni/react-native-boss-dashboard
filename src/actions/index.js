...
export const updateTask = (taskId, task, isDone) => {
  console.log('in action, taskId: ', taskId)
  console.log('in action, task: ', task)
  console.log('in action, isDone: ', isDone)
  if (isDone)
...
export const addEmployee = ({ name, phone, role, salary }) => {
  return dispatch => {
    dispatch({
      type: 'employee_adding_started'
    })
    firebase.database().ref(`/users/${UID}/employees`)
      .push({ name, phone, role, salary })
      .then(() => {
        dispatch({
...
export const updateEmployeeInfo = ({ name, phone, role, salary, uid }) => {
  return dispatch => {
    dispatch({
...
export const addMoneyAccount = ({ name, status, amount, amount1, amount2, amount3, amount4 }) => {
  return dispatch => {
    dispatch({
      type: 'account_adding_started'
    })
    firebase.database().ref(`/users/${UID}/money`)
      .push({ name, status, amount, amount1, amount2, amount3, amount4 })
      .then(() => {
        dispatch({
          type: 'account_reset'
        })
...
export const updateAccountInfo = ({ name, status, amount, amount1, amount2, amount3, amount4, uid }) => {
  return dispatch => {
    dispatch({
      type: 'account_updating_started'
    })
    firebase.database().ref(`users/${UID}/money/${uid}`)
      .set({ name, status, amount, amount1, amount2, amount3, amount4 })
      .then(() => {
        dispatch({
          type: 'account_reset'
        })
